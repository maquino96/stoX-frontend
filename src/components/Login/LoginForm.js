...
const LoginForm = ({isHidden, setIsHidden, formData, setFormData, updateForm}) => {
  ...
  const handleLogout = (e) => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}/batch/${defaultUser.watchlists.default.arrayList.join(',')}`)
          .then( r => r.json())
          .then( data => {
            console.log(data)
            dispatch(updateBatchWatchlist(data))
  ...
  return (
    <div style={{padding: '1em'}}>
      ...
      <Container>
        ...
        <Button onClick={()=>setIsHidden(!isHidden)}> Sign up </Button>
        </>
        </>
